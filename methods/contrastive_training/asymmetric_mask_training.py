...
from data.asymmetric_mask_dataset import AsymmetricMaskDataset, MergedAsymmetricMaskDataset
from models.asymmetric_foreground_encoder import AsymmetricForegroundEncoder
...
def train( projection_head, model, train_loader, test_loader, unlabelled_train_loader, merge_train_loader, args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader)):
            ...
            images, class_labels, uq_idxs, mask_lab, foreground_masks = batch
            mask_lab = mask_lab[:, 0]

            class_labels, mask_lab = class_labels.to(device), mask_lab.to(device).bool()
            global_images, foreground_images = (view.to(device) for view in images)
            foreground_masks = foreground_masks.to(device)

            # The global and foreground views share the ViT but have distinct token inputs.
            features = model(global_images, foreground_images, foreground_masks)
...
def extract_labeled_protos(model, train_loader, args):
    ...
    for batch_idx, (images, label, uq_idx, mask_lab_, foreground_masks) in enumerate(tqdm(train_loader)):
        ...
        feats = model.foreground_features(images, foreground_masks)
...
def test_kmeans(model, test_loader, epoch,
    ...
    for batch_idx, (images, label, _, foreground_masks) in enumerate(tqdm(test_loader)):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mask_root', type=str, required=True,
                        help='Root directory containing TokenCut .npy masks mirroring image-relative paths.')
    parser.add_argument('--max_foreground_tokens', default=128, type=int)
    ...
    # DATASETS
    # --------------------
    _, _, _, datasets = get_datasets(args.dataset_name, None, None, args)
    labelled_dataset = AsymmetricMaskDataset(datasets['train_labelled'], args.mask_root, train=True,
                                             image_size=args.image_size, crop_pct=args.crop_pct)
    unlabelled_dataset = AsymmetricMaskDataset(datasets['train_unlabelled'], args.mask_root, train=True,
                                               image_size=args.image_size, crop_pct=args.crop_pct)
    train_dataset = MergedAsymmetricMaskDataset(labelled_dataset, unlabelled_dataset)
    test_dataset = AsymmetricMaskDataset(datasets['test'], args.mask_root, train=False,
                                         image_size=args.image_size, crop_pct=args.crop_pct)
    unlabelled_train_examples_test = AsymmetricMaskDataset(datasets['train_unlabelled'], args.mask_root,
                                                           train=False, image_size=args.image_size,
                                                           crop_pct=args.crop_pct)

    model = AsymmetricForegroundEncoder(model, args.max_foreground_tokens, args.min_foreground_tokens).to(device)
    ...
    torch.cuda.empty_cache()
    if args.report:
        print("Skipping legacy reporting: it cannot reconstruct the required TokenCut foreground masks.")
